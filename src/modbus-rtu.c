...
ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    if (ctx_rtu->rtsfd >= 0) {
        gpio_set_value(ctx_rtu->rtsfd, 1);
	if (ctx->debug) {
	    printf("Setting RTS to write mode\n");
	}       	
    }
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    */
    ...
    if (ctx_rtu->rtsfd == -1) {
        ctx_rtu->rtsfd = gpio_export();
	if (ctx_rtu->rtsfd < 0) {
	    return -1;
	}
	gpio_set_dir(ctx_rtu->rtsfd, 1);
    }
...
void _modbus_rtu_close(modbus_t *ctx)
{
    ...
    if (ctx_rtu->rtsfd >= 0) {
        if (ctx->debug) 
	    printf("Close RTS control fd %d\n", ctx_rtu->rtsfd);	
	gpio_unexport(ctx_rtu->rtsfd);
    }
}
                       ...
                       struct timeval *tv, int length_to_read)
{
    ...
    if (ctx_rtu->rtsfd >= 0) {
        if (ctx->debug) {
	    printf("Setting RTS fd %d to read mode\n", ctx_rtu->rtsfd);	
	}
	if (gpio_set_value(ctx_rtu->rtsfd, 0) && ctx->debug)
	    printf("Setting RTS to read mode failed\n");
    }
